...
//#region lib/types/host/nudge.js
/**
...
const MAX_NUDGE_PROMPT_CHARS = 8e3;
/**
...
*/
function resolveSessionNudgeRoot() {
...
*/
function assertNudgeSessionId(sessionId) {
	if (!/^[A-Za-z0-9._-]{1,200}$/u.test(sessionId)) throw new Error("sessionId is not a safe nudge filename");
	return sessionId;
}
/**
...
*/
function resolveNudgePath(nudgeRoot, sessionId) {
...
*/
function normalizeNudgePrompt(raw) {
	const trimmed = raw.trim();
	if (trimmed.length === 0) return DEFAULT_NUDGE_PROMPT;
	if (trimmed.length > 8e3) throw new Error(`nudge prompt exceeds ${String(MAX_NUDGE_PROMPT_CHARS)} characters`);
	return trimmed;
}
/**
...
*/
function normalizeNudgeActiveHours(raw) {
	if (!Number.isSafeInteger(raw) || raw < 0 || raw > 720) throw new Error(`nudge activeHours must be an integer from 0 to ${String(720)}`);
	return raw;
}
/**
...
function parseNudgeState(raw) {
	...
	if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("nudge state must be a JSON object");
	const record = parsed;
	const enabled = record["enabled"] === true;
	const prompt = normalizeNudgePrompt(typeof record["prompt"] === "string" ? record["prompt"] : DEFAULT_NUDGE_PROMPT);
	const hoursRaw = record["activeHours"];
	return {
		enabled,
		prompt,
		activeHours: hoursRaw === void 0 ? 0 : typeof hoursRaw === "number" ? normalizeNudgeActiveHours(hoursRaw) : (() => {
...
* @param sessionId - Session id.
*/
async function readSessionNudge(nudgeRoot, sessionId) {
	const file = resolveNudgePath(nudgeRoot, sessionId);
	let raw;
	try {
		raw = await readFile(file, "utf8");
	} catch (error) {
		if (error instanceof Error && "code" in error && error.code === "ENOENT") return emptyNudgeState();
		throw error;
	}
...
*/
async function writeSessionNudge(nudgeRoot, sessionId, state) {
	const file = resolveNudgePath(nudgeRoot, sessionId);
...
*/
async function listEnabledNudgeSessionIds(nudgeRoot, nowMs = Date.now()) {
	let names;
	try {
...
*/
function nudgeRemainingMs(state, nowMs) {
...
* @param nowMs - evaluation clock.
*/
function isNudgeBudgetActive(state, nowMs) {
...
*/
function expireNudgeIfNeeded(state, nowMs, nowIso) {
	if (!state.enabled || isNudgeBudgetActive(state, nowMs)) return {
		state,
		expired: false
	};
	return {
...
* @param nowMs - evaluation clock.
*/
function shouldWakeNudge(state, agentStatus, nowMs) {
...
* @returns next state.
*/
function patchSessionNudge(previous, patch, nowIso) {
	if (patch.enabled === void 0 && patch.prompt === void 0 && patch.activeHours === void 0) throw new Error("nudge patch requires enabled, prompt, and/or activeHours");
	const enabled = patch.enabled ?? previous.enabled;
	const activeHours = patch.activeHours !== void 0 ? normalizeNudgeActiveHours(patch.activeHours) : previous.activeHours;
	const prompt = patch.prompt !== void 0 ? normalizeNudgePrompt(patch.prompt) : previous.prompt;
...
*/
function recordNudgePoll(state, nowIso) {
	return {
		...state,
		lastPolledAt: nowIso,
		updatedAt: nowIso
...
*/
function recordNudgeWake(state, nowIso) {
	return {
		...state,
		lastPolledAt: nowIso,
		lastWakeAt: nowIso,
...
function statusPayload(limits, nudge, connect) {
	return {
...
async function loadLiveNudge(nudgeRoot, sessionId) {
	let nudge;
	try {
...
async function handleAgentKernelStatus(req, res, sessions, limits) {
	...
	writeJson(res, 200, statusPayload(limits, await loadLiveNudge(limits.nudgeRoot, resolved.sessionId), await readAgentKernelConnect()));
}
async function handleAgentKernelNudge(req, res, sessions, limits) {
	if (!isTrustedApiRequest(req, limits.trustedHosts)) {
		res.writeHead(403).end("forbidden");
		return;
	}
...
async function handleAgentKernelNudgeIndex(req, res, limits) {
	if (!isTrustedApiRequest(req, limits.trustedHosts)) {
		res.writeHead(403).end("forbidden");
		return;
	}
