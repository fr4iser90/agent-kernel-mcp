...
import { DEFAULT_FOLLOWUP_PROMPT } from "../host/idle-followup.js";
...
function parseEntry(body, settingsOpen) {
    return {
        ...
        watchdogIntervalMinutes: typeof body.watchdogIntervalMinutes === 'number' && Number.isSafeInteger(body.watchdogIntervalMinutes)
...
export class AgentKernelHeaderController {
    ...
    polls = new Map();
    disposed = false;
    constructor(fetcher = (input, init) => fetch(input, init), pollMs = 5_000) {
    ...
    async dispose() {
        this.disposed = true;
        for (const sessionId of [...this.polls.keys()])
