 ...
 */
export function formatNudgeCountdownLabel(enabled, armedAt, activeHours, nowMs, foreverLabel) {
    if (!enabled)
        return '';
 ...
 */
export function formatNudgeNextCheckLabel(remainingMs, soonLabel) {
    if (remainingMs === null)
        return '';
    if (remainingMs <= 0)
        return soonLabel;
