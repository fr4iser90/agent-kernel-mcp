...
import { emptyNudgeState, expireNudgeIfNeeded, listEnabledNudgeSessionIds, patchSessionNudge, readSessionNudge, writeSessionNudge, } from "./nudge.js";
...
export async function handleAgentKernelNudge(req, res, sessions, limits) {
    if (!isTrustedApiRequest(req, limits.trustedHosts)) {
        res.writeHead(403).end('forbidden');
        return;
    }
...
export async function handleAgentKernelNudgeIndex(req, res, limits) {
    if (!isTrustedApiRequest(req, limits.trustedHosts)) {
        res.writeHead(403).end('forbidden');
        return;
    }
