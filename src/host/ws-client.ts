...
import { readAgentKernelConnect } from './connect.ts'
import { executeKernelJob } from './jobs.ts'
import { executorWsUrl } from './ws-protocol.ts'
...
let status: ExecutorWsStatus = {
  connected: false,
  lastError: null,
  lastHelloAt: null,
}

export function getExecutorWsStatus(): ExecutorWsStatus {
  return { ...status }
}
...
export function reconnectExecutorWs(): void {
  forceReconnect?.()
}
...
async function handleJob(ws: WebSocket, job: JobCreated): Promise<void> {
...
export function startExecutorWsClient(opts?: {
  ...
  const connect = async (): Promise<void> => {
    ...
    socket.on('message', (data: WebSocket.RawData) => {
      ...
      try {
        msg = JSON.parse(String(data)) as { type?: string } & Record<string, unknown>
      } catch {
        return
      }
      if (msg.type === 'hello') {
      ...
      if (msg.type === 'job.created') {
        const job = msg as unknown as JobCreated
