...
export function isCliExecutorId(id: string): id is CliExecutorId {
  return id === 'claude-code' || id === 'aider' || id === 'opencode'
}

function binFor(executorId: CliExecutorId): string {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('error', (err: NodeJS.ErrnoException) => {
      ...
      if (err.code === 'ENOENT') {
        reject(
          new Error(
            `${command} not found on PATH — install the CLI or set AGENT_KERNEL_${command.toUpperCase()}_BIN`,
          ),
        )
        return
      }
...
function composePrompt(objective: string, rolePromptText: string | null): string {
  const role =
    rolePromptText && rolePromptText.trim()
      ? `\n\n---\n# Role / Lawpack (injected)\n\n${rolePromptText.trim()}\n---\n`
      : ''
  return `${objective}${role}`
}

function parseClaudeJson(stdout: string): { text: string; sessionId: string | null } {
...
): Promise<{ text: string; externalSessionId: string | null }> {
...
async function invokeAider(cwd: string, prompt: string): Promise<string> {
...
}): Promise<{ executorSessionId: string }> {
...
export async function cliFetchTranscript(executorSessionId: string): Promise<Record<string, unknown>> {
