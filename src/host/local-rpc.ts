...
function resolveLocalHost(): LocalHostConfig {
  const endpoint = (
    process.env.DSH_HOST_URL?.trim() ||
...
export async function localRpc<T>(method: string, payload: Record<string, unknown>): Promise<T> {
...
export async function localCreateSession(cwd: string, agentPreset = 'standard'): Promise<string> {
  const value = await localRpc<{ sessionId?: string; id?: string }>('session.create', {
    cwd,
    agentPreset,
  })
...
export async function localPrompt(sessionId: string, text: string): Promise<void> {
...
export async function localListSessions(): Promise<{
  items: Array<{
    sessionId: string
    updatedAt: number
    running: boolean
...
export async function localHistoryAll(sessionId: string, maxPages = 20): Promise<{
  ...
  for (let i = 0; i < maxPages; i++) {
    const page = await localRpc<{
      events: typeof collected
      hasMore: boolean
    }>('session.history', {
      sessionId,
      maxMessages: 100,
