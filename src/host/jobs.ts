...
import { listWorkdirCandidates } from './detect-workdirs.ts'
import { cliContinueSession, cliFetchTranscript, cliStart, isCliExecutorId } from './cli-runners.ts'
import { readCliSession } from './cli-sessions.ts'
import {
  localCreateSession,
  localHistoryAll,
  localListSessions,
...
function executorIdFromPayload(payload: Record<string, unknown>): string {
...
function mapHistory(events: Array<{ event: { type: string; seq: number; time: number; data: unknown }; view?: unknown }>) {
  const messages: Array<Record<string, unknown>> = []
  const fileOps: Array<Record<string, unknown>> = []
...
async function runStartDsh(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  const brief = isRecord(payload.brief) ? payload.brief : null
  ...
  const roleBlock =
    typeof brief.rolePromptText === 'string' && brief.rolePromptText.trim()
...
async function runStartCli(
  executorId: 'claude-code' | 'aider' | 'opencode',
  payload: Record<string, unknown>,
  ...
  const objective =
    ...
    (typeof brief.initialObjective === 'string' && brief.initialObjective.trim()) ||
    `Obey Lawpack / AGENTS.md. RUN_ID=${String(brief.runId ?? '')}. Continue autonomous work.`
  const rolePromptText =
    typeof brief.rolePromptText === 'string' ? brief.rolePromptText : null
  return cliStart({ executorId, cwd, objective, rolePromptText })
}

async function runStart(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  const executorId = executorIdFromPayload(payload)
  if (isCliExecutorId(executorId)) return runStartCli(executorId, payload)
  if (executorId !== 'dsh') {
    throw new Error(
      `executorId=${executorId} is not implemented on this device — use dsh, claude-code, aider, or opencode`,
    )
  }
  return runStartDsh(payload)
}

async function runSessionContinueDsh(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
...
async function runSessionContinue(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  const sessionId = String(payload.executorSessionId ?? '').trim()
  if (!sessionId) throw new Error('session_continue job missing executorSessionId')
  const executorId = executorIdFromPayload(payload)
  const brief = isRecord(payload.brief) ? payload.brief : {}
...
async function runFetchTranscript(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  const sessionId = String(payload.executorSessionId ?? '').trim()
...
async function requireAgentPreset(presetId: string): Promise<void> {
  const listed = await localRpc<{ presets: Array<{ id: string }> }>('agentPreset.list', {})
  const ok = listed.presets.some((p) => p.id === presetId)
  if (!ok) {
    throw new Error(
      `DSH agent preset "${presetId}" not found — create it with tool deny (shell/edit) and only agent-kernel MCP tools`,
    )
  }
}

async function waitSessionIdle(sessionId: string, timeoutMs: number): Promise<void> {
...
function lastAssistantText(
  events: Array<{ event: { type: string; data: unknown } }>,
): string {
...
async function runOperatorTurn(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
...
export async function executeKernelJob(job: KernelJob): Promise<Record<string, unknown>> {
  ...
  if (job.kind === 'list_workdir_candidates') {
    ...
    if (Array.isArray(rootsRaw)) {
      const roots = rootsRaw.filter((r): r is string => typeof r === 'string')
      return listWorkdirCandidates({ roots })
    }
    return listWorkdirCandidates()
  }
  throw new Error(`unknown job kind: ${String((job as KernelJob).kind)}`)
}
