 ...
 */
export function formatAutonomyDuration(ms: number): string {
 ...
 */
export function formatAutonomyTimeLabel(
  runStartedAt: string,
  maxDurationMinutes: number,
  nowMs: number,
): string {
  if (runStartedAt.length === 0) return ''
  const started = Date.parse(runStartedAt)
  if (!Number.isFinite(started)) return ''
  const elapsed = formatAutonomyDuration(nowMs - started)
  if (!Number.isSafeInteger(maxDurationMinutes) || maxDurationMinutes <= 0) return elapsed
