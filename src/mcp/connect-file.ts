...
export function readConnectFile(): { url?: string; token?: string } {
  const candidates: string[] = []
  const ak = process.env.AGENT_KERNEL_HOME?.trim()
  if (ak) candidates.push(join(ak, 'connect.json'))
  const dsh = process.env.DSH_HOME?.trim()
  if (dsh) candidates.push(join(dsh, 'agent-kernel', 'connect.json'))
  candidates.push(join(homedir(), '.dsh', 'agent-kernel', 'connect.json'))
  candidates.push(join(homedir(), '.agent-kernel', 'connect.json'))

  for (const file of candidates) {
    try {
      const parsed = JSON.parse(readFileSync(file, 'utf8')) as { url?: unknown; token?: unknown }
