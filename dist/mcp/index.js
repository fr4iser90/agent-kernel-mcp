...
import { z } from 'zod';
import { readConnectFile } from "./connect-file.js";
function baseUrl() {
    ...
    throw new Error('AGENT_KERNEL_URL required (env or connect.json from: agent-kernel-runner pair --url … --code …)');
}
function token() {
    ...
    if (fromFile)
        return fromFile;
    throw new Error('AGENT_KERNEL_TOKEN required (env or connect.json from: agent-kernel-runner pair …)');
}
...
server.tool('ak_dispatch_run', 'Dispatch a run for an assignment (kernel starts / continues an executor session). Prefer this over ad-hoc DSH autonomy when the control plane owns the schedule.', { assignmentId: z.string() }, async ({ assignmentId }) => toolText(await api('POST', `/api/assignments/${encodeURIComponent(assignmentId)}/dispatch`, {})));
server.tool('ak_scheduler_tick', 'Run one agent-kernel scheduler tick (due assignments / dispatch).', {}, async () => toolText(await api('POST', '/api/scheduler/tick', {})));
...
server.tool('ak_detect_projects', 'Ask the paired device (WSS) for workdir candidates to register as projects. No kernel FS scan.', {}, async () => toolText(await api('POST', '/api/projects/detect', {})));
server.tool('ak_github_match', 'List GitHub repos and match against device workdirs (on_device vs missing). Requires GitHub login; never treats GitHub-only as executor-ready.', {}, async () => toolText(await api('POST', '/api/projects/github-match', {})));
